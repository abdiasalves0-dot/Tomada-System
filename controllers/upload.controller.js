...
exports.uploadBase64 = async (req, res) => {
  ...
  try {
    ...
    const dir = path.join(uploadsBaseDir, type);
...
exports.getLatestApk = async (req, res) => {
  try {
    const apkData = await googleDriveService.getLatestApkFromDrive();
    if (!apkData) {
      return res.json({
        success: false,
        message: 'Nenhum APK encontrado no Google Drive'
      });
    }
    ...
    res.json({
      ...
      filename: apkData.name,
      fileId: apkData.fileId,
      downloadUrl: `/api/upload/apk/download/${apkData.fileId}`
    });
  } catch (error) {
    console.error('❌ Erro no controller getLatestApk:', error.message);
...
exports.downloadApk = async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="Tomada-latest.apk"`);
    if (contentLength) {
      res.setHeader('Content-Length', contentLength);
    }
    ...
    console.error(`❌ Erro ao baixar APK ${fileId}:`, error.message);
    res.status(404).send('APK não encontrado no Google Drive');
  }
};
