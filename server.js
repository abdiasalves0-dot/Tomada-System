...
app.use('/storage/:type/:filename', async (req, res, next) => {
  ...
  const localPath = path.join(uploadsBaseDir, type, filename);
...
app.use('/uploads', express.static(process.env.VERCEL === '1' ? path.join('/tmp', 'uploads') : path.join(__dirname, 'uploads')));
...
let lastFtpErrorTime = 0;
const FTP_COOLDOWN = 10 * 60 * 1000; // Cooldown de 10 minutos após falha

async function loadFtpCatalog() {
  if (ftpCatalog) return ftpCatalog;
  
  // Se estiver no período de cooldown após um erro, evita nova tentativa bloqueante/pesada
  if (Date.now() - lastFtpErrorTime < FTP_COOLDOWN) {
    return {};
  }
  ...
  ftpCatalogLoading = (async () => {
    ...
    try {
      ...
      lastFtpErrorTime = Date.now();
      ftpCatalogLoading = null; // Reset para permitir tentar de novo após o cooldown
      return {};
    }
  })();
...
// Expõe o getter globalmente de forma síncrona imediata (sem bloquear a API)
global.getFtpCatalog = () => {
  if (!ftpCatalog && !ftpCatalogLoading && (Date.now() - lastFtpErrorTime >= FTP_COOLDOWN)) {
    // Dispara carregamento em background
    loadFtpCatalog().catch(() => {});
  }
  return ftpCatalog || {};
};

// Inicia o carregamento em background no startup do servidor (apenas fora do Vercel)
...
if (process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY) {
  webpush.setVapidDetails(
    process.env.VAPID_SUBJECT || 'mailto:admin@Bancada.com.br',
...
app.post('/api/push/notify', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const payload = JSON.stringify({
      title: '🍞 Bancada - Lembrete de Produção',
...
// Rodar a cada 2 horas (apenas fora do Vercel)
if (process.env.VERCEL !== '1') {
...
async function start() {
  ...
  httpServer.listen(PORT, HOST, () => {
    ...
    console.log('🍞 ══════════════════════════════════════════');
    console.log('   Bancada SISTEMA PADEIRO (PUBLIC IP ENABLED)');
    console.log('   ══════════════════════════════════════════');
