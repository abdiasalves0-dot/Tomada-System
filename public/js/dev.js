...
const Dev = {
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="card" style="background:#fff5f5; border-color:#feb2b2;">
            <div class="flex items-center gap-3 mb-3">
              ...
              <strong style="color:var(--danger)">Zerar Todos os Dados</strong>
            </div>
            <p class="text-secondary" style="font-size:13px; margin-bottom:16px;">Apaga completamente todas as tabelas e dados do sistema (exceto seu próprio usuário administrador).</p>
            <button class="btn btn-primary bg-danger border-none w-full" onclick="Dev.resetAllData()">
              <i data-lucide="trash-2"></i> Zerar Todos os Dados
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="flex gap-4 mb-4">
          ...
          <div class="form-group w-full">
            ...
            <select id="seed-filial" class="input-control">
              <option>Bancada Brasília</option>
              <option>Bancada Goiania</option>
              <option>Bancada Palmas</option>
              <option>Bancada Campo Grande</option>
            </select>
          </div>
        </div>
  ...
  async resetAllData() {
    if (confirm('Deseja realmente apagar ABSOLUTAMENTE TODOS os dados do sistema?')) {
      if (confirm('Atenção extrema: Esta ação é irreversível e apagará todos os padeiros, clientes, orçamentos, metas, cronogramas, atividades e históricos. Você continuará logado. Confirmar exclusão geral?')) {
        try {
          Components.toast('Apagando todos os dados...', 'info');
          await API.delete('/api/admin/reset-all-data');
          Components.toast('Todos os dados foram resetados com sucesso!', 'success');
          this.render();
        } catch(e) { Components.toast(e.message || 'Erro ao resetar dados', 'error'); }
      }
    }
  }
};
