...
const PadeiroFlow = {
    ...
    c.innerHTML = `
      <div class="pf-highlight-card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; flex-wrap: wrap; gap: 8px;">
          <label class="pf-label" style="color: #E55A2B; font-size: 14px; font-weight: 800; display: flex; align-items: center; gap: 8px; margin: 0; text-transform: uppercase; letter-spacing: 0.5px;">
        ...
        <div class="pf-select-wrap">
          ...
          <div class="pf-select-lock" style="color: #E55A2B;"><i data-lucide="${canSelect ? 'chevron-down' : 'lock'}" style="width:14px;height:14px"></i></div>
        </div>
      </div>
    ...
    c.innerHTML = `
      ...
      <div class="pf-dashboard-grid pf-animate-cascade" style="animation-delay: 0.10s">
        ...
        <div class="pf-dash-card">
          <div class="pf-dash-card-label"><i data-lucide="dollar-sign" style="width:14px;height:14px"></i> Valor Total</div>
          <div class="pf-dash-card-value">
            <span class="pf-dash-card-unit" style="margin-right:2px;">R$</span>
            <span id="flow-wallet-kg-display">${this.activity.kgTotal ? parseFloat(this.activity.kgTotal).toFixed(2).replace('.', ',') : '0,00'}</span>
          </div>
        </div>
  ...
  openCartModal() {
    ...
    let totalKgStr = totalKg > 0 ? `R$ ${totalKg.toFixed(2).replace('.', ',')}` : '--';
  ...
  calculateTotals() {
    ...
    if (displayKg) displayKg.innerText = totalKg > 0 ? totalKg.toFixed(2).replace('.', ',') : '0,00';
  ...
  saveDraftLocally() {
    ...
    try {
      localStorage.setItem('Bancada_padeiro_draft', JSON.stringify(draft));
    } catch(e) {
      console.warn("localStorage quota excedida, salvando sem fotos", e);
      const draftNoFotos = { totalKg, totalL, items, fotosBase64: [] };
      localStorage.setItem('Bancada_padeiro_draft', JSON.stringify(draftNoFotos));
    }
  },

  restoreDraftLocally() {
    const draftStr = localStorage.getItem('Bancada_padeiro_draft');
    if (!draftStr) return;
    try {
  ...
  async saveProducao() {
    ...
    try {
      ...
      localStorage.removeItem('Bancada_padeiro_draft');
  ...
  stepAvaliarCliente(c) {
    c.innerHTML = `
      <div class="pf-step-header">
        <div class="pf-step-icon" style="background: rgba(229, 90, 43, 0.1); color: #E55A2B;"><i data-lucide="smile" style="width:24px;height:24px"></i></div>
        <div>
  ...
  stepFinalizar(c) {
    ...
    c.innerHTML = `
      ...
      <div class="pf-summary">
        ...
        ${this.activity.kgTotal > 0 ? `<div class="pf-summary-row"><span>Valor Produzido</span><strong>R$ ${parseFloat(this.activity.kgTotal).toFixed(2).replace('.', ',')}</strong></div>` : ''}
      ...
      <div class="pf-container fade-in" style="max-width:500px;margin:60px auto;text-align:center;">
        <div class="pf-success-card">
          ...
          <div class="pf-success-stats" style="flex-wrap:wrap;gap:16px;">
            ${this.activity.kgTotal > 0 ? `<div class="pf-stat"><span class="pf-stat-val">R$ ${parseFloat(this.activity.kgTotal).toFixed(2).replace('.', ',')}</span><span class="pf-stat-label">Valor</span></div><div class="pf-stat-divider"></div>` : ''}
  ...
  startTutorial() {
    ...
    const key = 'Bancada_tutorial_producao_v10';
  ...
  _endTutorial() {
    localStorage.setItem('Bancada_tutorial_producao_v10', '1');
...
const PushService = {
  async init() {
    ...
    try {
      ...
      const resp = await fetch(baseUrl + '/api/push/vapid-public-key');
  ...
  async notifyAllInactive() {
    try {
      const token = localStorage.getItem('token') || API.token;
      const baseUrl = API.getBaseUrl ? API.getBaseUrl() : '';
      const resp = await fetch(baseUrl + '/api/push/notify', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
