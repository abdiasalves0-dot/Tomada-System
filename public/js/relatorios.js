...
window.Relatorios = {
  ...
  renderContent(c) {
    ...
    <div class="fade-in relatorios-view">
      ...
      <div class="grid grid-cols-2 gap-6 mb-8 charts-grid">
        <div class="card">
          <h3 class="card-title"><i data-lucide="bar-chart-3"></i> Produção por Funcionário</h3>
    ...
    return `
        ...
        <div class="flex justify-between items-start">
          <div>
            ...
            <h2 class="summary-value" style="font-size: 20px; line-height: 1.2;">
              <span style="color:#E55A2B;">R$ ${totalKg.toFixed(2).replace('.', ',')}</span>
              <span style="color:#7A7567; font-size:14px; font-weight:400; margin: 0 2px;">/</span>
              <span style="color:#FF9A3C;">${totalLitros.toFixed(1)} <span style="font-size: 11px; font-weight: 500;">L</span></span>
            </h2>
          </div>
      ...
      <div class="card summary-card">
        <div class="flex justify-between items-start">
          ...
          <div class="summary-icon" style="background: rgba(229, 90, 43, 0.1); color: #E55A2B;">
      ...
      <div class="card summary-card">
        <div class="flex justify-between items-start">
          ...
          <div class="summary-icon" style="background: rgba(255, 154, 60, 0.1); color: #FF9A3C;">
  ...
  renderRankingTable() {
    ...
    const ranking = Object.values(stats).map(s => {
      ...
      // Score calculation: (valor/100) + (avg*2) + (metas*5)
      const score = (s.kg / 100) + (avg * 2) + (s.metas * 5);
      return { ...s, avg, score };
    ...
    return ranking.map(r => `
      <tr>
        ...
        <td>
          <div style="color:#E55A2B; font-size:13px; font-weight:700;">R$ ${r.kg.toFixed(2).replace('.', ',')}</div>
          <div style="color:#FF9A3C; font-size:11px; font-weight:600; margin-top:2px;">${r.litros.toFixed(1)} L</div>
        </td>
        <td>
      ...
      return `
        <tr>
          ...
          <td>
            ...
            <span style="font-size: 10px; color: var(--text-tertiary);">R$ ${(m.produzido || 0).toFixed(2).replace('.', ',')} / R$ ${(m.quantidade || 0).toFixed(2).replace('.', ',')}</span>
          </td>
  ...
  renderVisitasSummary() {
    ...
    return Object.values(cliVisits).slice(0, 5).map(v => `
      <tr>
        ...
        <td><span style="font-weight: 700; color: #1C1A14;">${v.lastNota ? Number(v.lastNota).toFixed(1) : '—'}</span></td>
      </tr>
        ...
        datasets: [
          {
            label: 'Produção (R$)',
            data: valuesProdKg,
            backgroundColor: 'rgba(229, 90, 43, 0.8)',
            borderColor: '#E55A2B',
            borderWidth: 1,
            borderRadius: 6,
            barPercentage: 0.45,
          ...
          },
          {
            label: 'Produção (L)',
            data: valuesProdL,
            backgroundColor: 'rgba(255, 154, 60, 0.8)',
            borderColor: '#FF9A3C',
            borderWidth: 1,
            borderRadius: 6,
            barPercentage: 0.45,
