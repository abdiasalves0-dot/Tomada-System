...
Object.assign(Cronograma, {
  ...
  async onDropKanban(e, newKanbanListId) {
    e.preventDefault();
    e.stopPropagation(); // Evita propagar para o onDrop da lista
    ...
    const oldKanbanListId = task.kanbanListId;
    const oldPosicao = task.posicao;

    if (oldKanbanListId === newKanbanListId) return; // No change

    task.kanbanListId = newKanbanListId;
    
    const siblings = this.tarefas
      .filter(t => t.kanbanListId === newKanbanListId)
      .sort((a, b) => (a.posicao || 0) - (b.posicao || 0));
    
    task.posicao = siblings.length > 0 ? (siblings[siblings.length - 1].posicao || 0) + 10 : 0;

    this.renderSemanal();

    try {
      const updated = await API.put(`/api/cronograma/${task.id}`, { kanbanListId: newKanbanListId, posicao: task.posicao });
      const index = this.tarefas.findIndex(t => t.id === task.id);
      if (index !== -1 && updated) {
      ...
      task.kanbanListId = oldKanbanListId;
      task.posicao = oldPosicao;
      this.renderSemanal();
    }
  },
  async onDropKanbanDate(e, newDate) {
    e.preventDefault();
    e.stopPropagation();
    const taskId = e.dataTransfer.getData('text/plain') || this.draggedTaskId;
    if (!taskId) return;
    ...
    try {
      const updated = await API.put(`/api/cronograma/${task.id}`, { data: newDate, posicao: task.posicao });
      const index = this.tarefas.findIndex(t => t.id === task.id);
      if (index !== -1 && updated) {
        this.tarefas[index] = updated;
      }
      this.renderSemanal();
      Components.toast('Data atualizada!', 'success');
    } catch (err) {
      Components.toast('Erro ao atualizar data: ' + err.message, 'error');
      // Revert optimistic update
      task.data = oldDate;
      task.posicao = oldPosicao;
      this.renderSemanal();
  ...
  onDragListStart(e, listId) {
    if (e.target.closest('.trello-card')) {
      return;
    }
  ...
  onDragListEnd(e) {
    ...
    document.querySelectorAll('.trello-list').forEach(el => el.classList.remove('drag-over-list'));
  },

  onDragListOver(e) {
    if (this.draggedListId) {
      e.preventDefault();
      e.dataTransfer.dropEffect = 'move';
      const listEl = e.currentTarget.closest('.trello-list');
      if (listEl && listEl.dataset.listId !== this.draggedListId) {
  ...
  async onDropList(e, targetListId) {
    e.preventDefault();
    e.stopPropagation();
    const sourceListId = e.dataTransfer.getData('text/plain/list') || this.draggedListId;
    if (!sourceListId || sourceListId === targetListId) return;

    const sourceIdx = this.kanbanLists.findIndex(l => l.id === sourceListId);
    const targetIdx = this.kanbanLists.findIndex(l => l.id === targetListId);
    if (sourceIdx === -1 || targetIdx === -1) return;

    // Reordenar a lista localmente
    const [movedList] = this.kanbanLists.splice(sourceIdx, 1);
    this.kanbanLists.splice(targetIdx, 0, movedList);

    // Reatribuir posições sequenciais
    this.kanbanLists.forEach((list, idx) => {
      list.posicao = idx;
    });
    ...
    try {
      // Salvar todas as novas posições no banco em paralelo
      await Promise.all(this.kanbanLists.map(list => 
        API.put(`/api/kanban-lists/${list.id}`, { 
          titulo: list.titulo, 
          cor: list.cor, 
          posicao: list.posicao 
        })
      ));
      Components.toast('Listas reordenadas!', 'success');
    } catch (err) {
      Components.toast('Erro ao reordenar listas: ' + err.message, 'error');
      this.render(); // Restaura o estado original do banco se falhar
    }
  }
});
