...
Object.assign(Cronograma, {
  ...
  openMonthDetails(year, monthIndex) {
    ...
    const tableRows = padeirosAtivos.map(p => {
      const cells = dias.map(dia => {
        const tarefa = this.tarefas.find(t => Cronograma.isTaskOnDate(t, dia.dateStr) && t.padeiroId === p.id);
  ...
  renderMensal() {
    ...
    const activeTasks = this.tarefas.filter(t => t.status !== 'concluida');
    const activeClientsMap = new Map();
    activeTasks.forEach(t => {
      if (t.clienteId) {
        activeClientsMap.set(t.clienteId, t.clienteNome || '—');
      } else if (t.clienteNome) {
        activeClientsMap.set(t.clienteNome, t.clienteNome);
      }
    });
    const activeClientsList = Array.from(activeClientsMap.entries()).map(([id, nome]) => {
      const clientObj = this.clientes ? this.clientes.find(c => c.id === id || c.nome === nome) : null;
      return {
        id,
        nome,
        cidade: clientObj ? clientObj.cidade : null
      };
    });
    const totalClientesAtivos = activeClientsMap.size;

    // Weeks calculation for targetYear and monthIndex
    const totalDays = new Date(targetYear, monthIndex + 1, 0).getDate();
    const daysMap = {};
    for (let w = 1; w <= 6; w++) {
      daysMap[w] = [null, null, null, null, null, null, null]; // SEG, TER, QUA, QUI, SEX, SÁB, DOM
    }
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      const weekdayIdx = dow === 0 ? 6 : dow - 1; // 0 = SEG, 1 = TER, ..., 6 = DOM
    ...
    const selectedTasks = this.tarefas.filter(t => Cronograma.isTaskOnDate(t, selectedDateStr));
    ...
    const dayCellsHtml = gridDays.map(day => {
      ...
      const tasksOnDay = this.tarefas.filter(t => Cronograma.isTaskOnDate(t, dateStr));
      ...
      tasksListHtml = selectedTasks.map((t, idx) => {
        ...
        <div class="m-task-item cascade-item" style="--index: ${idx + 3};" onclick="Cronograma.openTaskDetail('${t.id}')">
          ...
          <div class="m-task-details">
            <span class="m-task-cat">${statusLabel} • Funcionário: ${bakerName}</span>
            <span class="m-task-name">${t.nome || t.tarefas || t.clienteNome || 'Tarefa sem nome'}</span>
    ...
    <div class="tf-container desktop-only">
      <!-- Top Header -->
      <div class="tf-header">
        <div class="tf-brand" style="display: flex; align-items: center; gap: 10px; height: 38px;">
          ${App.getLogoSvg(true)}
        </div>
        ...
        <div class="tf-controls">
          ...
          <div class="tf-top-stats">
            ...
            <div class="tf-stat-item">
              <div class="tf-stat-label">Clientes Ativos</div>
              <div class="tf-stat-val">${totalClientesAtivos}</div>
            </div>
          </div>
        ...
        <div class="tf-sidebar">
          ...
          <div class="tf-checklist-card cascade-item" style="--index: 2;">
            <div class="tf-card-header">
              <span class="tf-card-title">Clientes Ativos</span>
              <span class="tf-card-link" onclick="App.navigate('clientes')">Ver todos</span>
            </div>
            <div class="tf-checklist tf-checklist-scrollable">
              ${activeClientsList.map(c => `
                <div class="tf-check-item">
                  <div class="tf-check-circle" style="border-color: #E55A2B; background: #E55A2B; color: #FFFFFF;">
                    <i data-lucide="check" style="width: 10px; height: 10px;"></i>
                  </div>
                  <span title="${c.nome}">${c.nome.split(' ').slice(0, 2).join(' ')} ${c.cidade ? `(${c.cidade})` : ''}</span>
                </div>
              `).join('') || `
                <div style="font-size: 12px; color: #94A3B8; padding: 4px 0;">Nenhum cliente ativo</div>
              `}
            </div>
          </div>
        ...
        <div class="tf-board-container">
          <div class="tf-board-header cascade-item" style="--index: 2;">
            ...
            <div style="display: flex; align-items: center; gap: 16px;">
              <span class="tf-card-link" style="color: #E55A2B;" onclick="Cronograma.weekOffset=0; Cronograma.render(); return false;">Hoje</span>
  ...
  _renderMonthlyGrid(activeWeeks, daysMap, selectedWeekSet, todayStr) {
    ...
    <div class="tf-timeline-grid cascade-item" style="--index: 3;">
      <div class="tf-grid-header">
        ...
        <div class="tf-header-cell"><span class="tf-header-day-num" style="font-size: 14px;">DOM</span><span class="tf-header-day-name">Domingo</span></div>
      </div>
      ...
      ${activeWeeks.map((weekNum, idx) => {
        ...
        <div class="tf-grid-row cascade-item" style="--index: ${idx + 4};">
          ...
          ${weekDays.map(day => {
            ...
            const tasksInCell = this.tarefas.filter(t => Cronograma.isTaskOnDate(t, day.dateStr));
              ...
              <div style="display: flex; flex-direction: column; gap: 8px; overflow-y: auto; max-height: 140px;">
                ${tasksInCell.map((t, idx) => {
                  ...
                  <div class="tf-task-card ${cardClass}" onclick="Cronograma.openTaskDetail('${t.id}')" style="min-height: 50px; padding: 6px 8px; border-radius: 8px;">
                    <span class="tf-card-title" title="${t.nome || t.tarefas || t.clienteNome}" style="font-size: 10px; font-weight: 700;">${t.nome || t.tarefas || t.clienteNome || '—'}</span>
  ...
  _renderDailyGrid(dates, todayStr) {
    ...
    const dayTasks = this.tarefas.filter(t => Cronograma.isTaskOnDate(t, selectedDateStr))
      ...
      <div class="tf-daily-slot cascade-item" style="--index: ${idx + 4};">
        ...
        <div class="tf-daily-slot-content ${hasTask ? '' : 'empty'}">
          ${tasks.map((t, tIdx) => {
            ...
            const cardClass = isEven ? 'tf-card-slate' : 'tf-card-lime';
            let statusDot = '#1C1A14';
            if (t.status === 'concluida') statusDot = '#34C759';
            else if (t.status === 'em_andamento') statusDot = '#E55A2B';
            return `
            <div class="tf-daily-task-card ${cardClass}" onclick="Cronograma.openTaskDetail('${t.id}')">
              <div class="tf-daily-task-top">
                ...
                <span class="tf-daily-task-client">${t.nome || t.tarefas || t.clienteNome || '—'}</span>
              </div>
  ...
  _renderWeeklyGrid(dates, todayStr) {
    ...
    const columnsHtml = dates.map((date, i) => {
      ...
      const dayTasks = this.tarefas.filter(t => Cronograma.isTaskOnDate(t, dateStr))
        .sort((a, b) => (a.horario || '08:00').localeCompare(b.horario || '08:00'));
      ...
      <div class="tf-weekly-column ${isToday ? 'today' : ''} cascade-item" style="--index: ${i + 4};">
        ...
        <div class="tf-weekly-col-tasks">
          ...
          ${dayTasks.map((t, tIdx) => {
            ...
            const isEven = tIdx % 2 === 0;
            const cardClass = isEven ? 'tf-card-slate' : 'tf-card-lime';
            let statusColor = '#1C1A14';
            if (t.status === 'concluida') statusColor = '#34C759';
            else if (t.status === 'em_andamento') statusColor = '#E55A2B';
            return `
            <div class="tf-weekly-task ${cardClass}" onclick="Cronograma.openTaskDetail('${t.id}')">
              ...
              <div class="tf-weekly-task-body">
                <span class="tf-weekly-task-name">${t.nome || t.tarefas || t.clienteNome || '—'}</span>
