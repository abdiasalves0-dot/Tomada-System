...
const PadeiroAgenda = {
    ...
    style.innerHTML = `
      @media (max-width: 480px) {
        ...
        .fab-add {
          ...
          height: 56px;
          border-radius: 28px;
          background: var(--Bancada-orange, #E55A2B);
          color: white;
          display: flex;
          align-items: center;
          justify-content: center;
  ...
  async render() {
    ...
    if (!this.selectedFilial) {
      this.selectedFilial = user.filial || 'Bancada Brasília';
    }
    ...
    container.innerHTML = `
      <div class="agenda-mobile-view fade-in">
        <div class="flex justify-between items-center mb-6" style="animation: pfCascadeUp 0.5s cubic-bezier(0.16, 1, 0.3, 1) both; animation-delay: 0s;">
          ...
          <div class="badge badge-primary">${String(this.selectedFilial || 'Bancada').split(' ')[1] || 'Bancada'}</div>
        </div>
  ...
  renderTaskCardMobile(t, index = 0) {
    ...
    if (name.includes('massa') || name.includes('prep')) category = { name: 'Massa', icon: 'chef-hat', color: '#E55A2B' };
    else if (name.includes('assar') || name.includes('forno')) category = { name: 'Assar', icon: 'flame', color: '#1C1A14' };
